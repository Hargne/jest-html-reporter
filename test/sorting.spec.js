...
describe('sorting', () => {
	it('should have no effect when sort not configured', () => {
		...
		const sortedTestResults = sorting.sortSuiteResults({ testData: testResults, sortMethod: undefined });
	...
	describe('byStatus', () => {
		...
		it('should order results as 1.pending 2.failed 3.passed', () => {
			const { testResults } = mockdata.jestResponse.multipleTestResults;
			const sortedTestResults = sorting.sortSuiteResults({ testData: testResults, sortMethod: 'status' });
	...
	describe('byExecutionDesc', () => {
		it('should order the test data by execution time', () => {
			const { testResults } = mockdata.jestResponse.multipleTestResults;
			const sortedTestResults = sorting.sortSuiteResults({ testData: testResults, sortMethod: 'executionDesc' });
	...
	describe('byExecutionAsc', () => {
		it('should order the test data by execution time', () => {
			const { testResults } = mockdata.jestResponse.multipleTestResults;
			const sortedTestResults = sorting.sortSuiteResults({ testData: testResults, sortMethod: 'executionAsc' });
	...
	describe('byTitleDesc', () => {
		it('should order the test data by name', () => {
			const { testResults } = mockdata.jestResponse.multipleTestResults;
			const sortedTestResults = sorting.sortSuiteResults({ testData: testResults, sortMethod: 'titleDesc' });

			expect(sortedTestResults[0].testFilePath).toEqual('index-c.js');
			expect(sortedTestResults[0].testResults[0].ancestorTitles[0]).toEqual('ancestor c');
			expect(sortedTestResults[0].testResults[1].ancestorTitles[0]).toEqual('ancestor b');
			expect(sortedTestResults[0].testResults[2].ancestorTitles[0]).toEqual('ancestor a');

			expect(sortedTestResults[1].testFilePath).toEqual('index-b.js');
			expect(sortedTestResults[1].testResults[0].ancestorTitles[0]).toEqual('ancestor c');
			expect(sortedTestResults[1].testResults[1].ancestorTitles[0]).toEqual('ancestor b');
			expect(sortedTestResults[1].testResults[2].ancestorTitles[0]).toEqual('ancestor a');

			expect(sortedTestResults[2].testFilePath).toEqual('index-a.js');
			expect(sortedTestResults[2].testResults[0].ancestorTitles[0]).toEqual('ancestor c');
			expect(sortedTestResults[2].testResults[1].ancestorTitles[0]).toEqual('ancestor b');
			expect(sortedTestResults[2].testResults[2].ancestorTitles[0]).toEqual('ancestor a');
		});
	});
	...
	describe('byTitleAsc', () => {
		it('should order the test data by name', () => {
			const { testResults } = mockdata.jestResponse.multipleTestResults;
			const sortedTestResults = sorting.sortSuiteResults({ testData: testResults, sortMethod: 'titleAsc' });

			expect(sortedTestResults[0].testFilePath).toEqual('index-a.js');
			expect(sortedTestResults[0].testResults[0].ancestorTitles[0]).toEqual('ancestor a');
			expect(sortedTestResults[0].testResults[1].ancestorTitles[0]).toEqual('ancestor b');
			expect(sortedTestResults[0].testResults[2].ancestorTitles[0]).toEqual('ancestor c');

			expect(sortedTestResults[1].testFilePath).toEqual('index-b.js');
			expect(sortedTestResults[1].testResults[0].ancestorTitles[0]).toEqual('ancestor a');
			expect(sortedTestResults[1].testResults[1].ancestorTitles[0]).toEqual('ancestor b');
			expect(sortedTestResults[1].testResults[2].ancestorTitles[0]).toEqual('ancestor c');

			expect(sortedTestResults[2].testFilePath).toEqual('index-c.js');
			expect(sortedTestResults[2].testResults[0].ancestorTitles[0]).toEqual('ancestor a');
			expect(sortedTestResults[2].testResults[1].ancestorTitles[0]).toEqual('ancestor b');
			expect(sortedTestResults[2].testResults[2].ancestorTitles[0]).toEqual('ancestor c');
		});
	});
});
